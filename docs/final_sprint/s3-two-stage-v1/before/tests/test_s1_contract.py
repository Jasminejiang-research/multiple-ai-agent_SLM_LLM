...
class FakeClient:
    ...
    def generate_structured_once(self, prompt, schema, **kwargs):
        ...
        role = next((role for model, role in roles.items() if issubclass(schema, model)), None)
        if role is not None:
...
def test_common_generation_path_and_first_research_evidence(condition, tmp_path):
    ...
    for (_, schema, _), base in zip(client.calls[-4:], CONTRACT_BATCH_MODELS, strict=True):
        assert issubclass(schema, base) and set(schema.model_fields) == set(base.model_fields)
        assert all(schema.model_fields[key].annotation == field.annotation and
                   schema.model_fields[key].is_required() == field.is_required()
...
def test_cross_batch_collision_fails_terminal_without_extra_repair():
    ...
    def inconsistent(prompt, schema, **kwargs):
        candidate = original(prompt, schema, **kwargs)
        if issubclass(schema, CONTRACT_BATCH_MODELS[-1]):
