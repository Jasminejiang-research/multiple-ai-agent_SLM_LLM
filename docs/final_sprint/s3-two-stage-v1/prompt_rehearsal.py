...
for case in ('ai_education','intelligent_ring'):
    ...
    workflow=build_review_workflow(ctx,config=config,provider=provider,output_dir=ROOT/'validation'/('synthetic_envelope_'+case))
    result=workflow.run()
...
path=ROOT/'validation/prompt_envelope.json'
