...
for case in ('ai_education','intelligent_ring'):
    ...
    workflow=build_review_workflow(ctx,config=config,provider=provider,output_dir=ROOT/'validation'/('synthetic_envelope_v3_'+case))
    result=workflow.run()
...
path=ROOT/'validation/prompt_envelope_v3.json'
