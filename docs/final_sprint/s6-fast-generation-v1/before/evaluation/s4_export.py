...
from evaluation.s4_io import contained,file_hash,read_csv,write_csv,write_json
...
METRICS = {"completion":"higher","first_contract":"higher","handoff":"higher",
...
def export_experiment(experiment,output,*,human_pack=None,figures=True):
    ...
    for name,direction in METRICS.items():
        d,s=paired(data["rows"],name,direction=direction)
        details.extend(d)
        summaries.extend(s)
