param(
    ...
    [string]$RunName = 'real_preflight_d_ai_education_03'
)
...
$priorResultPath = Join-Path $PSScriptRoot 'real_preflight_d_ai_education_02\probe\result.json'
if (-not (Test-Path -LiteralPath $priorResultPath)) {
    throw 'The preserved two-call pre-dispatch component failure is required before this bounded resume.'
}
$priorResult = Get-Content -LiteralPath $priorResultPath -Raw | ConvertFrom-Json
if ($priorResult.status -ne 'no_go' -or $priorResult.requested_phase -ne 'all' -or
        $priorResult.model_calls -ne 2 -or $priorResult.phases.warmup.status -ne 'passed' -or
        $priorResult.phases.context.status -ne 'passed' -or $priorResult.phases.components.status -ne 'failed' -or
        $priorResult.phases.smoke.status -ne 'not_run' -or
        $priorResult.phases.warmup.budget.request_count -ne 1 -or
        $priorResult.phases.warmup.budget.charged_total_tokens -ne 61 -or
        $priorResult.phases.context.budget.request_count -ne 1 -or
        $priorResult.phases.context.budget.charged_total_tokens -ne 12918 -or
        $priorResult.phases.components.probes.research.budget.request_count -ne 0 -or
        $priorResult.server_stop.status -ne 'owned_process_tree_terminated') {
    throw 'Resume refused: attempt 02 is not the exact preserved 2-call, 12,979-token pre-dispatch failure.'
}
try {
