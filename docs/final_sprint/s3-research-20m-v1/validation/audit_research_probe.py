...
The helper never imports a provider/runner, starts a model, repairs an artifact,
or writes under the target repository. Call once after probe/result.json exists.
...
def verify_accounting(summary, calls, audit, report_root):
    ...
    original = audit.json(record['result_path'])
    require(audit.files[str(Path(record['result_path']).resolve())]['sha256'] == record['sha256'], 'Carryover source hash differs')
    require(carry['source_result']['sha256'] == record['sha256'], 'Result substituted its carryover source')
    old, old_snapshot = original['budget'], original['budget']['per_plan_budget']
    require(carry['source_per_plan_budget'] == old_snapshot, 'Historical per-plan snapshot changed')
    require(carry['pool_limits'] == dict(request_count=36, charged_total_tokens=500000, elapsed_seconds=5400), 'Pool ceilings changed')
    for key in ('request_count', 'charged_total_tokens', 'elapsed_seconds'):
        require(carry['pool_used'][key] == old['this_run_' + key], 'Pool incorrectly based on all-history totals')
        require(carry['all_history_used'][key] == old['cumulative_' + key], 'Older history changed')
    ...
    for key, value in known.items():
        require(carry['pool_known_usage'][key] == old_snapshot[key], 'Historical known subtotal changed')
        require(summary['pool_cumulative']['known_usage'][key] == old_snapshot[key] + value, 'Pool known subtotal differs')
...
def audit_run(run, output, repo):
    ...
    report = dict(audit_version='research-probe-integrity-v1', created_at_utc=datetime.now(timezone.utc).isoformat(),
    ...
    report['history'] = audit.check('3279_historical_files_unchanged', lambda: check_inventory(audit,
        report_root / 'history_inventory.json', [repo], expected_count=3279))
    ...
    def verify_outcome():
        ...
        expected = min(1200, 5400 - inherited['pool_used']['elapsed_seconds'])
