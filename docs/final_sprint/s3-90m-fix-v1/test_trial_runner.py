...
import pytest
from trial_runner import RecoveryConfig, TrialBudget, verified_carryover, validate_authorization
...
def test_prior_run_and_all_phases_share_time_and_request_tokens():
    ...
    assert remaining['run_seconds'] == pytest.approx(5400-240)
...
def test_recovery_carryover_uses_cumulative_not_latest_usage(tmp_path):
    ...
    assert 5399<remaining['run_seconds']<=5400


def test_fresh_model_runs_have_independent_ninety_minute_clocks():
    now = [0.0]
    prior = dict(elapsed_seconds=4788.311, request_count=13, charged_total_tokens=191323)
    first = TrialBudget(RecoveryConfig(), prior, clock=lambda: now[0])
    now[0] = 5399
    assert first.remaining()['run_seconds'] == 1
    # A different model's run must not inherit this model's clock. Financial/request
    # authorizations are separate; this only exercises independent run deadlines.
    second = TrialBudget(RecoveryConfig(), prior, clock=lambda: now[0])
    assert second.remaining()['run_seconds'] == 5400
    now[0] = 5400
    with pytest.raises(ValueError, match='exhausted'):
        first.remaining()
    assert second.remaining()['run_seconds'] == 5399


def test_older_cumulative_allowance_is_not_reset_by_new_ninety_minute_limit():
    prior = dict(elapsed_seconds=14000, request_count=35, charged_total_tokens=499000)
    ledger = TrialBudget(RecoveryConfig(), prior, clock=lambda: 0)
    assert ledger.remaining() == dict(run_seconds=400, max_requests=1, max_total_tokens=1000)


def test_time_limit_cannot_be_raised_or_authorization_reused():
    with pytest.raises(ValueError): RecoveryConfig(run_seconds=5401)
    with pytest.raises(ValueError): RecoveryConfig(cumulative_trial_seconds=14401)
    with pytest.raises(ValueError, match='authorization'):
        validate_authorization(RecoveryConfig(), {'scope':'ai_education_slm_nonformal_4h','approved':True})
