...
class RecoveryConfig(CapabilityConfig):
    model_config_version: Literal['granite-h-micro-cpu-90m-v1'] = 'granite-h-micro-cpu-90m-v1'
    ...
    cumulative_trial_seconds: Literal[14400] = 14400


def validate_authorization(config, record):
    expected = {key: getattr(config, key) for key in ('run_seconds', 'node_seconds',
        'request_seconds', 'max_output_tokens', 'max_requests', 'max_total_tokens',
        'max_prompt_chars', 'cumulative_trial_seconds')}
    if record.get('scope') != 'ai_education_slm_nonformal_90m_recovery' or record.get('approved') is not True:
        raise ValueError('Explicit 90-minute trial authorization is missing')
...
def verified_carryover(record):
    ...
    if 'budget' in result and 'cumulative_request_count' in result['budget']:
        budget = result['budget']
        return dict(elapsed_seconds=budget['cumulative_elapsed_seconds'],
            request_count=budget['cumulative_request_count'],
...
class TrialBudget:
    ...
    def remaining(self):
        ...
        values = dict(run_seconds=min(self.config.run_seconds-elapsed,
            self.config.cumulative_trial_seconds-self.prior['elapsed_seconds']-elapsed),
    ...
    def summary(self):
        ...
        return dict(this_run_request_count=self.requests-self.prior['request_count'],
            ...
            this_run_time_limit_seconds=self.config.run_seconds,
            cumulative_time_limit_seconds=self.config.cumulative_trial_seconds)
...
def execute(config, authorization, prior, *, shared_repo, output, owner_path, tokenizer_path, tokenizer_provenance):
    ...
    result = dict(case_id='ai_education', formal_eligible=False, formal_runs_started=0, gemini_calls=0,
        ...
        per_plan_deadline_seconds=config.run_seconds,
        deadline_scope='this fresh SLM recovery including Research prerequisite; separate from any LLM run')
    def deadline():
        deadline_fired.set()
        cancel.set()
    ...
    try:
        ...
        if metadata['model_digest'] != 'eec81a822241037c5d8e47a870b3d26423bfa1b010e02fcdf1b5f9f2de5c3b2b' or metadata['ollama_version'] != '0.33.2':
            raise ValueError('Model digest or Ollama runtime differs from the approved prior trial')
