...
> 版本：v1.3-fast-generation-protocol｜初版日期：2026-09-04｜本次修订：2026-09-07｜项目：`multiple_ai_agent`  
...
## 0A. 2026-09-07 已批准的紧凑生成协议修订

本节是当前产品迭代的最高优先级设计；与下文旧的“两阶段正文/声明”、Granite 完整 canonical schema、单节点 20 分钟或全链 90 分钟条款冲突时，以本节及 `@ai_agent_evaluation_metrics_spec_v2_fast_generation.md` 为准。旧测试和版本说明作为历史证据保留。

已观察到的瓶颈是重型 grounding schema 和重复全量上下文：Research 正文曾以 4,399 输入 tokens、442 输出 tokens 在约 186 秒完成，但第二阶段未能在约 27 分钟内结束。当前优化不更换 Gemini 2.5 Flash 或 Granite 4.0 H Micro，不改变 A/B/C/D、两个 case、8 个 planned runs、角色集合、Critic 层级、Tier 1 权重或 B−A/C−B/C−D。

共同实现规则：

1. 每次调用的公共 system＋当前角色/任务指令合计不超过 500 个 Unicode 字符；完整动态 prompt、wire schema 和实际 tokens 分开记录。
2. 使用一次轻量 JSON 生成正文与 `[E01]` 短证据 ID；不使用 YAML，不再执行独立的模型声明/grounding 阶段。确定性代码无损展开完整来源元数据、claim lineage 与 canonical Pydantic 输出。
3. 完整输入与原始 artifact 继续归档；模型只接收由共同版本化规则生成的角色视图和交接胶囊。禁止新增模型摘要调用或 D-only pruning。
4. Critic 保留角色与固定评分，输出紧凑 score vector、状态和最多三个定向问题；低/中问题允许留存。失败门触发的唯一语义 Revision 改为字段/claim patch，不重写整份 artifact，也不增加第二轮语义 Critic。
5. Finance 的 21 行共同财务值由冻结输入和公式确定性产生；Finance Agent 负责假设、解释和风险。
6. C/D 在 provider 前保存相同的规范化 prompt、角色视图和 wire schema hash；Gemini/Granite 均使用同一轻量语义结构和严格本地 canonical 校验。
7. 30 分钟记录 `MVP30` 计划材料，60 分钟为共同 run 硬限。部分材料、超时和失败全部保留，不能冒充完整成功。

该修改在正式 0/8 时获批，属于正式实验前的共同协议迭代。实施后使用新 prompt/wire/context/config 版本重新冻结；旧协议结果只作工程诊断。
...
| 公平比较 | A–D 使用相同 brief、冻结 evidence、32K 上限、轻量 JSON wire protocol、确定性角色视图和 canonical 校验 | C/D 的规范化 prompt、字段语义和 wire schema 相同；provider 包装差异单独记录 |
...
| 生成速度 | 目标 30 分钟内形成 MVP30 计划材料，A–D 完整流程硬限 60 分钟 | 使用共同紧凑协议；低/中问题可留给质量评价，硬契约失败仍如实记录 |
...
| 指标 | 定义 | 来源 |
|---|---|---|
| Completion rate | 60分钟内完成条件规定流程并成功导出canonical输出 / 全部planned runs；失败保留在分母 | 自动 |
| MVP30 material rate | 30分钟内形成满足确定性最低材料契约的13章节计划材料 / 全部planned runs | 自动 |
| Time to first MVP30 material | run开始到首次通过MVP30契约；未达到记`not_reached` | 自动 |
| ValidPlan60 rate | 60分钟内完成Critic/路由、终端契约和canonical导出的比例 | 自动 |
| Time to first valid plan | run开始到完整计划首次通过统一终端契约；包含此前失败与修复耗时 | 自动 |
| Peak RAM / VRAM / pagefile | 分项记录每个run的实测峰值和采样完整性 | 自动 |
| Requests / tokens / retries | 包含失败、Critic、结构repair和语义Revision | 自动 |
| Prompt / schema burden | 指令字符、完整prompt字符、provider输入tokens、wire schema字节 | 自动 |
| Stage latency | prefill、generation、校验、展开、Critic、Revision、导出和清理 | 自动 |

`MVP30`只表示结构完整、财务值可复算、短引用可解析、事实/假设状态可追踪的计划材料已经形成；它不代表规定Critic全部完成或内容高质量。`ValidPlan60`要求条件规定的全部流程和统一终端契约完成。未完成run不把硬限当作成功时间，内容质量仍只由Academic Quality评价。
...
```text
revision_required = exists(high_or_critical_issue)
...
- Critic prompt 只含确定性角色要求、当前紧凑 artifact、其引用的证据片段和 source allowlist；
- 为 Writer/final Critic 预留请求预算；
- B/C/D在无blocking/revision-required issue时跳过final full revision；C/D的三个角色节点同样仅在检查失败时修订；
- 每个节点使用冻结的角色预算；30分钟记录MVP30，完整run达到60分钟或资源硬门即停止并保存现场。
...
SLM_STRUCTURED_MODE=compact_json_wire
SLM_MAX_PROMPT_CHARS=26000
SLM_MAX_OUTPUT_TOKENS=1800
SLM_REQUEST_TIMEOUT=720
SLM_NODE_BUDGET_SECONDS=720
SLM_RUN_BUDGET_SECONDS=3600

SLM_RUN_MAX_REQUESTS=18
SLM_CONTEXT_PROBE=1

SLM_CHUNKED_WRITER=0
SLM_PRUNED_SCHEMAS=1
```

`SLM_MAX_OUTPUT_TOKENS=1800`是Writer/Single的总上限；Research、Strategy、Finance、Critic和patch使用更低的角色上限。每次调用的timeout取角色上限、run剩余时间与资源门中的最小值。Writer以一次紧凑wire响应覆盖13章；模型不再执行独立grounding阶段。所有结构repair和语义Revision仍计入同一60分钟run预算。

Ollama 支持 JSON Schema structured output，但仍要用严格 Pydantic 校验，并最多允许一次结构纠正请求[^ollama-structured]。

S6启用共同的轻量wire schema与确定性角色视图：
...
2. **结构门**：分别测试一个 Research wire、紧凑`ComponentCritiqueReport`、Finance wire 和一次13章Writer wire；要求首轮或一次修正后通过wire校验，并能确定性展开为严格canonical Pydantic。
3. **时间门**：用真实最大prompt测量；各角色按冻结目标预算运行，30分钟记录MVP30状态，完整Multi-Agent硬限60分钟。达到资源或run硬限即停止；tokens/s只作诊断，不作为GO/NO-GO门。
...
同一case的A–D使用完全相同的brief、冻结evidence packet、共同32K上限、轻量JSON wire protocol、确定性角色视图、temperature、适用角色输出上限和一次结构纠错机会。Confidence/provenance与canonical校验同样一致。
...
- 对A–D使用共同32K context、60分钟run硬限及适用角色的共同output budget；
- 同一规范化prompt版本、角色视图、轻量wire schema、canonical输出契约与校正机会；
...
本轮取消依赖固定修订对照的`3/100`质量与`15%`token节省阈值。C−D只报告相同优化Multi架构下的模型差异；不作模型×架构交互归因。SLM内存、结构、MVP30与ValidPlan60门按v1.3共同协议冻结。
...
两次Granite正式运行各以60分钟为硬限，本地运行最多2小时，另有6次Gemini正式运行；实际提前完成时间照实记录。按人工审核预留 **1小时**，本轮代码、实验与审核整体建议预留 **8–14小时**；这些是排期估计，须由SLM预检校准，不是实测承诺。预检与调试调用另计，不属于8次正式实验。
...
| SLM 超预算 | 达到冻结角色预算、完整run超过60分钟或资源硬门 | 停止对应D run并保存MVP/部分材料；模型变更须新配置和确认，不新增Single正式组，不声称完成缺失的C/D比较 |
...
- A–D统一使用轻量JSON wire protocol、确定性角色视图、短证据ID映射和canonical展开；C/D相同逻辑调用的规范化语义hash一致；
- 30分钟MVP30与60分钟ValidPlan60/Completion状态均可从日志复算；
