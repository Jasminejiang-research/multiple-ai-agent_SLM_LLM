...
from schemas.compact_wire import CriticWire, PatchWire, ROLE_WIRES
from workflow.compact_protocol import compact_role_output
...
class SyntheticProvider:
    ...
    def invoke(self, request):
        ...
        if request.schema is PatchWire:
            view = inputs["view"]
            issue = view["issues"][0]
            if issue.get("claims"):
                data = {"claims": [{"issue": issue["i"],
                    "i": issue["claims"][0], "c": "low",
                    "cr": "critic_unresolved"}], "fields": []}
            else:
                path = issue["fields"][0]
                data = {"claims": [], "fields": [{"issue": issue["i"],
                    "path": path, "value": view["fields"][path]}]}
            return PhysicalResponse(json.dumps(data), dict(synthetic=True, prompt=10, output=5, total=15), 10, 5, 15)
        if request.schema is CriticWire:
            role = inputs["role"].removesuffix("_critic")
            severity = self.severity if role in self.fail_roles else None
            artifact_role = "writer" if role == "final" else role
            path = {"research": "market_trends.0.finding",
                    "strategy": "value_proposition.0.recommendation",
                    "finance": "revenue_assumptions.0.assumption",
                    "final": "executive_summary.content"}[role]
            issues = [] if severity is None else [{"i": f"{role}.issue1", "sev": severity,
                "criterion": METRICS[role][0], "claims": ["research.pricing"],
                "fields": [path], "e": [], "fix": "retain_limit",
                "note": "Synthetic issue; support still needs human verification."}]
            data = {"scores": [self.score if role in self.fail_roles else 3 for _ in METRICS[role]],
                    "status": "FAIL" if severity in ("high", "critical") else "PASS",
                    "issues": issues}
            return PhysicalResponse(json.dumps(data), dict(synthetic=True, prompt=10, output=5, total=15), 10, 5, 15)
        compact_role = next((role for role, schema in ROLE_WIRES.items() if request.schema is schema), None)
        if compact_role:
            version = 1
            canonical = synthetic_artifact(self.context, compact_role, version=version, inputs=inputs)
            canonical = ROLE_SCHEMAS[compact_role].model_validate(canonical)
            data = compact_role_output(self.context, compact_role, canonical).model_dump(mode="json")
            return PhysicalResponse(json.dumps(data), dict(synthetic=True, prompt=10, output=5, total=15), 10, 5, 15)
