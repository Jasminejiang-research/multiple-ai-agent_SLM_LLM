...
def reduce_run(directory, *, condition):
    ...
    result, config, branches, first_complete, first_mvp, first_valid = None, {}, {}, None, None, None
    ...
    for event in events:
        ...
        if kind == "call":
            ...
            if isinstance(elapsed,(int,float)):
                first_complete = elapsed if first_complete is None else min(elapsed,first_complete)
        elif kind == "mvp30_material" and body.get("passed"):
            elapsed = body.get("elapsed_seconds")
            if isinstance(elapsed,(int,float)):
                first_mvp = elapsed if first_mvp is None else min(elapsed,first_mvp)
        elif kind == "valid_plan60" and body.get("passed"):
            elapsed = body.get("elapsed_seconds")
            if isinstance(elapsed,(int,float)):
                first_valid = elapsed if first_valid is None else min(elapsed,first_valid)
        elif kind == "mechanical_repair":
            mechanical_repairs.append(body)
        elif kind == "telemetry":
            samples.append(body)
        elif kind == "route":
            routes.append(body)
    ...
    if result and first_mvp is None:
        first_mvp = result.get("first_mvp30_material_seconds")
    if result and first_complete is None:
        first_complete = result.get("first_complete_contract_seconds") or result.get("first_valid_plan_seconds")
    burden_fields = ("instruction_chars", "full_prompt_chars", "full_prompt_utf8_bytes", "wire_schema_bytes")
    burden = {field+"_max": max((c[field] for c in call_list if isinstance(c.get(field),int)), default=None)
              for field in burden_fields}
    burden["instruction_limit_passed"] = bool(call_list) and all(
        isinstance(c.get("instruction_chars"), int) and c["instruction_chars"] <= 500 for c in call_list)
    burden["normalized_prompt_hashes"] = sorted({c["normalized_prompt_hash"] for c in call_list if c.get("normalized_prompt_hash")})
    burden["role_view_hashes"] = sorted({c["role_view_hash"] for c in call_list if c.get("role_view_hash")})
    stage_latency = defaultdict(float)
    for call in call_list:
        if isinstance(call.get("elapsed_seconds"),(int,float)):
            stage_latency[f"{resource_role(call)}:{call.get('purpose','unknown')}"] += call["elapsed_seconds"]
    return dict(condition=condition,result=result,config=config,events=events,attempts=call_list,
        ...
        observed_first_complete_contract_seconds=first_complete,
        first_mvp30_material_seconds=first_mvp,
        first_mvp30_material_state="observed" if first_mvp is not None else "not_reached",
        mvp30_material_reached=first_mvp is not None,
        valid_plan60_reached=completed and first_valid is not None and first_valid <= config.get("run_seconds",3600),
        valid_plan60_state="observed" if completed and first_valid is not None else "not_reached",
        artifact_class=(result or {}).get("artifact_class", "partial_or_failed"),
        prompt_schema_burden=burden, stage_latency_seconds=dict(stage_latency),
        deterministic_mechanical_repair_count=len(mechanical_repairs),
        deterministic_mechanical_repairs=mechanical_repairs,
