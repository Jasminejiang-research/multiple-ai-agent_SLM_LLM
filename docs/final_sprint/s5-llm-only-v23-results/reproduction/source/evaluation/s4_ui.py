...
def render_experiment_panel(st):
    ...
    st.info("S6 compact protocol view. MVP30 is material availability; ValidPlan60 requires the condition's full Critic/route/terminal path.")
    ...
    try:
        ...
        st.json({k:reduced[k] for k in ("mvp30_material_reached","first_mvp30_material_seconds",
            "valid_plan60_reached","first_valid_plan_seconds","artifact_class","first_contract","handoff",
            "review_token_share","request_count","total_tokens","thinking_tokens","prompt_schema_burden",
            "stage_latency_seconds","deterministic_mechanical_repair_count","peak_ram_bytes",
            "peak_vram_bytes","peak_pagefile_bytes")})
