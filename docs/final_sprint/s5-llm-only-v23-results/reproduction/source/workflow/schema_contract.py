...
def _finite_members(fragment, root_schema, *, depth=0):
    """Return JSON literals only when every alternative is a closed scalar set."""
    if depth > MAX_TRAVERSAL_DEPTH or not isinstance(fragment, Mapping):
        return None
    if '$ref' in fragment:
        resolved = _resolve_local_ref(root_schema, fragment['$ref'])
        return _finite_members(resolved, root_schema, depth=depth+1)
    if 'const' in fragment:
        values = [fragment['const']]
    elif isinstance(fragment.get('enum'), list):
        values = fragment['enum']
    elif fragment.get('type') == 'null':
        values = [None]
    else:
        alternatives = fragment.get('anyOf', fragment.get('oneOf'))
        if not isinstance(alternatives, list):
            return None
        result = []
        for alternative in alternatives:
            members = _finite_members(alternative, root_schema, depth=depth+1)
            if members is None:
                return None
...
def _collect_enums(
    ...
    *,
    ...
    if enum_values is None and "const" in fragment:
        enum_values = [fragment["const"]]
    if path and isinstance(enum_values, list) and enum_values:
        output.append(
            (".".join(path), tuple(json.dumps(value, ensure_ascii=False, separators=(',', ':')) for value in enum_values))
        )
    ...
    for union_key in ("anyOf", "oneOf", "allOf"):
        ...
        if isinstance(alternatives, list):
            if union_key != 'allOf':
                members = _finite_members({union_key: alternatives}, root_schema)
                if path and members is not None:
                    output.append(('.'.join(path), members))
                    continue
                # An open branch means this field is not a closed enum. Keep
                # only nested-property constraints from the individual branches.
                branch_output = []
                for alternative in alternatives:
                    if isinstance(alternative, Mapping):
                        _collect_enums(alternative, root_schema, path, branch_output,
                            depth=depth+1, seen_refs=seen_refs)
                output.extend(item for item in branch_output if item[0] != '.'.join(path))
                continue
            for alternative in alternatives:
                if isinstance(alternative, Mapping):
                    _collect_enums(
