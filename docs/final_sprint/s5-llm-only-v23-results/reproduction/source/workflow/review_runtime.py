...
from workflow.compact_protocol import (INSTRUCTION_LIMIT, ROLE_INPUT_TOKENS,
                                       ROLE_SECONDS, GEMINI_8192_REPAIR_CONFIG_VERSION,
...
class BoundedClient:
    ...
    def __init__(self, provider, config, journal, *, cancel=None, clock=time.monotonic,
                 local_lease=None, planned_id=None, budget=None, started_at=None,
        ...
        if budget is None:
            budget = RunBudget(max_requests=config.max_requests, max_total_tokens=config.max_total_tokens)
        if not isinstance(budget, RunBudget):
            raise TypeError("budget must be a RunBudget instance")
        if (budget.max_requests, budget.max_total_tokens) != (config.max_requests, config.max_total_tokens):
            raise ValueError("shared budget limits must match the explicit run configuration")
        self.budget = budget
        ...
        self.node_role = None
        self.node_effective_seconds = None
        self.input_refs = []
        self.attempt_counts = {}
    ...
    def snapshot(self):
        ...
        now = self.clock()
        node_deadline = (None if self.node_started is None else
            self.node_started - self.started + self.node_effective_seconds)
        effective_deadline = min(self.config.run_seconds,
            self.config.run_seconds if node_deadline is None else node_deadline)
        ...
        return {**budget, "charged_total_tokens": charge, "token_counter_basis": "budget_charge_actual_or_retained_estimate",
            ...
            "run_elapsed_seconds": now - self.started,
            "node_elapsed_seconds": None if self.node_started is None else now - self.node_started,
            "node_scope_id": self.node_scope_id, "node_role": self.node_role,
            "node_effective_seconds": self.node_effective_seconds,
            "node_deadline_run_elapsed_seconds": node_deadline,
            "effective_deadline_run_elapsed_seconds": effective_deadline,
            "effective_request_timeout_seconds": max(0, min(self.config.request_seconds,
                effective_deadline - (now - self.started))),
            "research_profile_version": RESEARCH_PROFILE_VERSION,
            "research_total_seconds": min(self.config.node_seconds, RESEARCH_TOTAL_SECONDS),
            "research_output_caps": {stage: min(self.config.max_output_tokens, cap)
                for stage, cap in RESEARCH_OUTPUT_CAPS.items()},
            "configured_max_output_tokens": self.config.max_output_tokens,
            "configured_thinking_budget": self.config.thinking_budget,
            "effective_max_output_tokens": None if metadata is None else self._output_cap(metadata)}

    def _budget_role(self, metadata):
        if metadata["task_purpose"] == "revision":
            return "revision"
        role = metadata["role"]
        # Preflight-only probes exercise the same transport envelope without
        # adding experimental roles. Charge them against the largest common
        # single-call envelope so compact protocol accounting remains bounded.
        if role in ("warmup", "context_probe"):
            return "single"
        return role

    def _output_cap(self, metadata):
        cap = self.config.max_output_tokens
        if self.config.protocol_version == WIRE_VERSION:
            budget_role = self._budget_role(metadata)
            cap = min(cap, role_output_tokens(self.config.config_version)[budget_role])
            return cap
        if metadata["role"] == "research" and metadata["task_purpose"] in ("generate", "revision"):
            stage_cap = RESEARCH_OUTPUT_CAPS.get(metadata.get("generation_stage", "single"), RESEARCH_OUTPUT_CAPS["single"])
    ...
    def check(self):
        ...
        if self.node_started is not None:
            remaining = min(remaining, self.node_effective_seconds - (now - self.node_started))
        if remaining <= 0:
    ...
    def begin_node(self, role):
        ...
        self.node_role = role
        self.node_effective_seconds = (min(self.config.node_seconds, RESEARCH_TOTAL_SECONDS)
            if role == "research" else self.config.node_seconds)
    ...
    def attempt_scope(self, **metadata):
        ...
        try:
            ...
            outcome["first_physical_attempt_passed"] = self.first_physical_stage_pass.get((metadata["logical_task_id"], metadata.get("generation_stage")))
    ...
    def _charge(self, response, estimate, event):
        ...
        self.budget.record_usage(prompt_tokens=prompt if not missing else 0,
            output_tokens=output if not missing else 0, total_tokens=charge)
        event["budget_token_charge"] = charge

    def generate_structured_once(self, prompt, schema, *, temperature=0, system_instruction=None,
                                 output_validator=None, repair_evidence_catalog=None,
                                 repair_financial_ids=None):
        metadata = _ATTEMPT.get()
        ...
        output_cap = self._output_cap(metadata)
        estimate = len((combined + schema_json).encode("utf-8")) + output_cap
        if self.config.protocol_version == WIRE_VERSION:
            instruction_chars = len(system_instruction or "")
            if instruction_chars > INSTRUCTION_LIMIT:
                self.journal.emit("blocked_task", dict(**metadata, reason="instruction_limit",
                    instruction_chars=instruction_chars, limit=INSTRUCTION_LIMIT))
                raise SharedBudgetStop("compact instruction character limit exceeded")
            budget_role = self._budget_role(metadata)
            estimated_input_tokens = (len((combined + schema_json).encode("utf-8")) + 3) // 4
        ...
        for transport_index in range(self.config.transport_retries + 1):
            ...
            try:
                ...
                if self.cumulative_budget:
                    self.cumulative_budget.check_request(estimated_tokens=estimate)
                self.budget.reserve_request(estimated_tokens=estimate)
                if self.cumulative_budget:
                    self.cumulative_budget.reserve_request(attempt_id=attempt_id,
                        estimated_tokens=estimate, journal=self.journal)
            ...
            event = dict(run_id=self.journal.run_id, planned_id=self.planned_id, run_kind=self.config.run_kind,
                ...
                generation_stage=metadata.get("generation_stage", "single"),
                ...
                normalized_prompt_hash=metadata.get("normalized_prompt_sha256"),
                role_view_hash=metadata.get("role_view_sha256"),
                instruction_chars=len(system_instruction or ""),
                full_prompt_chars=len(combined), full_prompt_utf8_bytes=len(combined.encode("utf-8")),
                wire_schema_bytes=len(schema_json.encode("utf-8")),
                estimated_input_tokens=estimated_input_tokens if self.config.protocol_version == WIRE_VERSION else None,
                role_input_target_tokens=ROLE_INPUT_TOKENS[budget_role] if self.config.protocol_version == WIRE_VERSION else None,
                role_input_target_estimate_passed=(estimated_input_tokens <= ROLE_INPUT_TOKENS[budget_role]) if self.config.protocol_version == WIRE_VERSION else None,
                mechanical_repair_applied=False, mechanical_repair_diff=[],
            ...
            try:
                budget_role = self._budget_role(metadata)
                role_seconds = ROLE_SECONDS[budget_role] if self.config.protocol_version == WIRE_VERSION else self.config.request_seconds
                request = PhysicalRequest(prompt, schema, system_instruction, 0, output_cap,
                    min(remaining, self.check(), self.config.request_seconds, role_seconds),
                    thinking_budget=self.config.thinking_budget)
                ...
                repair_diff = []
                citation_list_format_repair = (
                    self.config.config_version in (
                        GEMINI_8192_CITATION_FORMAT_CONFIG_VERSION,
                        GEMINI_8192_GAP_COUNT_CONFIG_VERSION,
                        GEMINI_8192_EXACT_CARDINALITY_CONFIG_VERSION,
                        GEMINI_8192_FINANCE_MARKER_CONFIG_VERSION,
                ...
                financial_marker_repair = (
                    self.config.config_version in (
                        ...
                        GEMINI_8192_FINANCE_NOTICE_CONFIG_VERSION,
                        GEMINI_8192_REPAIR_INSTRUCTION_CONFIG_VERSION, GEMINI_8192_LINEAGE_ID_CONFIG_VERSION, GEMINI_8192_ID_COLLISION_CONFIG_VERSION, GEMINI_8192_DUAL_LINEAGE_CONFIG_VERSION))
                source_quality_scale_5_repair = (
                    self.config.config_version in (GEMINI_8192_SOURCE_QUALITY_CONFIG_VERSION,
                        GEMINI_8192_BODY_ASSEMBLY_CONFIG_VERSION,
                        GEMINI_8192_FINANCE_CAPACITY_CONFIG_VERSION,
                        GEMINI_8192_FINANCE_PARTITION_CONFIG_VERSION,
                        GEMINI_8192_FINANCE_NOTICE_CONFIG_VERSION,
                        GEMINI_8192_REPAIR_INSTRUCTION_CONFIG_VERSION, GEMINI_8192_LINEAGE_ID_CONFIG_VERSION, GEMINI_8192_ID_COLLISION_CONFIG_VERSION, GEMINI_8192_DUAL_LINEAGE_CONFIG_VERSION))
                existing_claim_body_assembly_repair = (
                    self.config.config_version in (GEMINI_8192_BODY_ASSEMBLY_CONFIG_VERSION,
                        GEMINI_8192_FINANCE_CAPACITY_CONFIG_VERSION,
                        GEMINI_8192_FINANCE_PARTITION_CONFIG_VERSION,
                        GEMINI_8192_FINANCE_NOTICE_CONFIG_VERSION,
                        GEMINI_8192_REPAIR_INSTRUCTION_CONFIG_VERSION, GEMINI_8192_LINEAGE_ID_CONFIG_VERSION, GEMINI_8192_ID_COLLISION_CONFIG_VERSION, GEMINI_8192_DUAL_LINEAGE_CONFIG_VERSION))
                source_disposition_repair = self.config.config_version in (
                    GEMINI_8192_CITATION_CONTRACT_CONFIG_VERSION,
                    GEMINI_8192_CITATION_FORMAT_CONFIG_VERSION,
                    GEMINI_8192_GAP_COUNT_CONFIG_VERSION,
                    GEMINI_8192_EXACT_CARDINALITY_CONFIG_VERSION,
                    ...
                    GEMINI_8192_FINANCE_NOTICE_CONFIG_VERSION,
                    GEMINI_8192_REPAIR_INSTRUCTION_CONFIG_VERSION, GEMINI_8192_LINEAGE_ID_CONFIG_VERSION, GEMINI_8192_ID_COLLISION_CONFIG_VERSION, GEMINI_8192_DUAL_LINEAGE_CONFIG_VERSION)
                punctuation_repair = self.config.config_version in (
                    GEMINI_8192_PUNCTUATION_REPAIR_CONFIG_VERSION,
                    GEMINI_8192_PREMISE_CONFIG_VERSION,
                    GEMINI_8192_TEXT_EQUALS_CLAIM_CONFIG_VERSION,
                    GEMINI_8192_CITATION_CONTRACT_CONFIG_VERSION,
                    ...
                    GEMINI_8192_FINANCE_NOTICE_CONFIG_VERSION,
                    GEMINI_8192_REPAIR_INSTRUCTION_CONFIG_VERSION, GEMINI_8192_LINEAGE_ID_CONFIG_VERSION, GEMINI_8192_ID_COLLISION_CONFIG_VERSION, GEMINI_8192_DUAL_LINEAGE_CONFIG_VERSION)
                conservative_repair = punctuation_repair or (
                    self.config.config_version == GEMINI_8192_REPAIR_CONFIG_VERSION)
                finance_partition_recency_repair = (
                    self.config.config_version in (GEMINI_8192_FINANCE_PARTITION_CONFIG_VERSION,
                        GEMINI_8192_FINANCE_NOTICE_CONFIG_VERSION,
                        GEMINI_8192_REPAIR_INSTRUCTION_CONFIG_VERSION, GEMINI_8192_LINEAGE_ID_CONFIG_VERSION, GEMINI_8192_ID_COLLISION_CONFIG_VERSION, GEMINI_8192_DUAL_LINEAGE_CONFIG_VERSION))
                try:
                    ...
                    if self.config.protocol_version == WIRE_VERSION:
                        try:
                            candidate, repair_diff = mechanical_repair_json(response.text, schema,
                                conservative_claim_repair=conservative_repair,
                                terminal_punctuation_claim_repair=punctuation_repair,
                                source_disposition_claim_repair=source_disposition_repair,
                                citation_list_format_claim_repair=citation_list_format_repair,
                                inline_citation_mirror_claim_repair=inline_citation_mirror_repair,
                                financial_marker_claim_repair=financial_marker_repair,
                                source_quality_scale_5_claim_repair=source_quality_scale_5_repair,
                                existing_claim_body_assembly_claim_repair=existing_claim_body_assembly_repair,
                                invalid_source_recency_claim_repair=finance_partition_recency_repair,
                                finance_note_partition_repair=finance_partition_recency_repair,
                                finance_assumption_notice_repair=(self.config.config_version in (GEMINI_8192_FINANCE_NOTICE_CONFIG_VERSION, GEMINI_8192_REPAIR_INSTRUCTION_CONFIG_VERSION, GEMINI_8192_LINEAGE_ID_CONFIG_VERSION, GEMINI_8192_ID_COLLISION_CONFIG_VERSION, GEMINI_8192_DUAL_LINEAGE_CONFIG_VERSION)),
                    ...
                    # Exact-span punctuation repair is a canonical precondition, not a DTO
                    # precondition, so v9 must evaluate it even when the raw DTO is valid.
                    if punctuation_repair:
                        candidate, repair_diff = mechanical_repair_json(response.text, schema,
                            ...
                            repair_evidence_catalog=repair_evidence_catalog,
                            repair_financial_ids=repair_financial_ids)
                event["mechanical_repair_applied"] = bool(repair_diff)
                event["mechanical_repair_diff"] = repair_diff
                if repair_diff:
                    self.journal.emit("mechanical_repair", dict(attempt_id=attempt_id,
                        schema_hash=metadata["schema_sha256"], changes=repair_diff,
                        before_sha256=canonical_hash(response.text),
                ...
                if self.cumulative_budget:
                    self.cumulative_budget.record_usage(attempt_id=attempt_id,
                        event=event, journal=self.journal)
