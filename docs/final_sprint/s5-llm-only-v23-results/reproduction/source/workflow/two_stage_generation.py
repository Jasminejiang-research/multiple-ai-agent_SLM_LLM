...
class SelectionPlan:
    def __init__(self, canonical, body, context, *, upstream=(), previous=None,
        ...
        for number, (path, spec, parent_model, parent) in enumerate(_claim_nodes(canonical, self.body), 1):
            ...
            claim_contract = get_args(spec.annotation)[0]
            if not isinstance(claim_contract, type) or not issubclass(claim_contract, GroundedClaim):
                raise TypeError('Claim collections require a GroundedClaim contract')
            claim_fields = {k:(v.annotation,deepcopy(v)) for k,v in claim_contract.model_fields.items()
            ...
            evidence_limit = len(self.evidence_spans)
            if getattr(canonical, '__research_profile__', None):
                # One atomic Research claim normally needs at most two anchors.
                # Never make an inherited exact anchor set impossible to retain.
                inherited_max = max((len(row['evidence_span_ids']) for rows in self.inherited_claim_evidence.values()
                    for row in rows), default=0)
                evidence_limit = min(evidence_limit, max(2, inherited_max))
            claim_fields["evidence_span_ids"] = (list[evidence_id_type],
                Field(max_length=evidence_limit, json_schema_extra={"uniqueItems":True}))
