...
from workflow.compact_protocol import PROMPT_VERSION as COMPACT_PROMPT_VERSION
from workflow.compact_protocol import protocol_manifest
...
class ReviewWorkflow:
    def __init__(self, context, *, config, provider, output_dir, run_id=None, planned_id=None,
                 ...
                 shared_budget=None, started_at=None, cumulative_budget=None):
        ...
        if session_factory:
            ...
            with session_factory() as session:
                create_run(session, run_id=self.run_id, workflow_version=config.workflow_version,
                    prompt_version=COMPACT_PROMPT_VERSION if config.protocol_version == WIRE_VERSION else PROMPT_VERSION,
                    model_name=config.model_exact_id, input_brief=context.brief)
                session.commit()
        ...
        self.client = BoundedClient(provider, config, self.journal, cancel=cancel, local_lease=local_lease,
            planned_id=planned_id, budget=shared_budget, started_at=started_at,
            cumulative_budget=cumulative_budget, **kwargs)
        ...
        self.first_complete_contract_seconds = None
        self.first_mvp30_material_seconds = None
        ...
        compact_manifest = protocol_manifest(config.config_version) if config.protocol_version == WIRE_VERSION else None
        self.journal.emit("run", dict(status="created", config=config.model_dump(mode="json"),
            ...
            compact_protocol=compact_manifest,
            compact_protocol_sha256=canonical_hash(compact_manifest) if compact_manifest else None))
    ...
    def _save_artifact(self, role, stage, artifact):
        ...
        if role in ("single", "writer"):
            self.client.check()
            elapsed = self.client.clock() - self.client.started
            if self.first_complete_contract_seconds is None:
                self.first_complete_contract_seconds = elapsed
            within_mvp = elapsed <= self.config.mvp_seconds
            if within_mvp and self.first_mvp30_material_seconds is None:
                self.first_mvp30_material_seconds = elapsed
            self.journal.emit("complete_plan_contract", dict(passed=True, artifact_ref=ref,
                elapsed_seconds=elapsed, stage=stage))
            self.journal.emit("mvp30_material", dict(passed=within_mvp,
                deterministic_contract_passed=True, elapsed_seconds=elapsed,
                deadline_seconds=self.config.mvp_seconds, artifact_ref=ref,
                artifact_class="MVP30" if within_mvp else "complete_contract_after_MVP30_deadline"))
        return ref
        ...
        def node(state):
            try:
                ...
                if name.endswith(".gate"):
                    role = name.split(".")[0]
                    route.update(rule="any high/critical issue; score is diagnostic only",
    ...
    def _terminal(self):
        ...
        elapsed = self.client.clock()-self.client.started
        self.first_valid_plan_seconds = elapsed
        self.journal.emit("terminal_contract", dict(passed=True, artifact_ref=self._ref(f"{role}.effective"),
            elapsed_seconds=elapsed))
        self.journal.emit("valid_plan60", dict(passed=elapsed <= self.config.run_seconds,
            elapsed_seconds=elapsed, deadline_seconds=self.config.run_seconds,
            artifact_ref=self._ref(f"{role}.effective"), artifact_class="ValidPlan60"))
    ...
    def _finish(self, state):
        ...
        self.result = dict(run_id=self.run_id, status=self.status, error=self.error, workflow_version=self.config.workflow_version,
            ...
            first_complete_contract_seconds=self.first_complete_contract_seconds,
            first_mvp30_material_seconds=self.first_mvp30_material_seconds,
            mvp30_state="reached" if self.first_mvp30_material_seconds is not None else "not_reached",
            valid_plan60_state="reached" if self.terminal_contract_passed and self.first_valid_plan_seconds is not None and self.first_valid_plan_seconds <= self.config.run_seconds else "not_reached",
            artifact_class="ValidPlan60" if self.terminal_contract_passed else "MVP30" if self.first_mvp30_material_seconds is not None else "partial_or_failed",
