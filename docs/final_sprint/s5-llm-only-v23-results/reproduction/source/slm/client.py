        ...
        *,
        ...
        if not isinstance(content, str) or not content.strip():
            raise StructuredOutputValidationError(
                f"SLM returned an empty response for {schema.__name__}.", raw_output=content if isinstance(content, str) else None
            )
        ...
        try:
            ...
            raise StructuredOutputValidationError(
                f"Invalid {schema.__name__} output: {exc}", raw_output=content
            ) from exc
