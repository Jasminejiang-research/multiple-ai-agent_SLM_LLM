# Final sprint S3 entry

The versioned Granite H Micro integration is `python -m slm.granite_preflight`.
See `docs/final_sprint/s3-v1/HANDOFF.md` for model setup, full shared D workflow,
explicit 32768 context, telemetry, limits and current feasibility status.
The legacy instructions below retain their historical configuration and are not
the S3/final-experiment entry point.
