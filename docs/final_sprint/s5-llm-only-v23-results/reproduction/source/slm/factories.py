...
def build_slm_contract_generator(client: SLMClient, *, task_sink=None):
    """S1/S3 seam: the exact shared complete schema and 4/3/3/3 implementation.

    The caller explicitly supplies the local client. No default Qwen client,
    SlimProposalSection, condense_prompt or nested batching is used here.
    Existing build_slm_adapters remains a legacy-only compatibility entry.
    """
    from workflow.contract_generation import build_contract_generator
    return build_contract_generator(client, task_sink=task_sink)


def build_slm_review_workflow(context, *, config, provider, local_lease, **kwargs):
    """S2/S3 injection: complete D graph via the common physical-call controller.

    S3 supplies a Granite provider with invoke(PhysicalRequest)->PhysicalResponse,
    SDK retries disabled, and a shared endpoint lease. Legacy SLMClient/adapters
    do not implement this surface and cannot silently supply Qwen or pruning.
    """
    if context.condition != "D" or config.condition != "D" or provider.provider != "local":
        raise ValueError("D requires an explicitly injected local physical provider")
    from workflow.review_graph import build_review_workflow
    return build_review_workflow(context, config=config, provider=provider, local_lease=local_lease, **kwargs)
