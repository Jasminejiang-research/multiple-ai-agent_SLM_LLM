...
@pytest.mark.parametrize("condition,expected_calls,reviewed", [("A", 4, ()), ("B", 8, ("final",)),
    ("C", 11, ("research", "strategy", "finance", "final")), ("D", 11, ("research", "strategy", "finance", "final"))])
...
def test_fail_routes_revise_once_and_keep_semantic_state(tmp_path, condition, role, severity, score):
    ...
    assert len(revisions) == (4 if role == "final" else 1)
    ...
    if role != "final":
        for req in provider.calls:
            ...
            for upstream in payload.get("upstream_artifacts", []):
                if upstream["role"] == gen_role and req.schema is not ComponentCritiqueReport:
...
def test_b_final_failure_and_all_c_failures(tmp_path):
    for condition in ("B", "C"):
        ...
        result = workflow.run()
        assert result["status"] == "completed", result["error"]
        assert len(provider.calls) == (12 if condition == "B" else 18)
...
@pytest.mark.parametrize("role,limit", [("research", 2), ("strategy", 4), ("finance", 6), ("final", 11)])
...
def test_transport_retry_is_a_physical_attempt_not_structure_repair(tmp_path):
    ...
    assert len(workflow.generator.tasks[0].attempts) == 1
...
def test_bad_critic_reference_never_reaches_revision(tmp_path, source_field):
    def fail(req, index):
        if req.schema is ComponentCritiqueReport:
    ...
    assert not any(c["task_purpose"] == "revision" for c in calls(workflow))
    assert len(provider.calls) == (2 if source_field == "source_ids" else 3)
...
def test_sqlite_existing_json_columns_persist_events_and_versions(tmp_path):
    ...
    with sessions() as session:
        ...
        assert run.workflow_version == "multi-agent-review-gates-v2" and run.status == "completed"
        events = [node.output_snapshot for node in run.node_outputs]
        assert len([e for e in events if e["kind"] == "call"]) == 24
        ...
        assert run.token_usage["request_count"] == 12
