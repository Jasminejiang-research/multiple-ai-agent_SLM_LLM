...
helpers = ['research_runner.py', 'cleanup_owned.py', 'Invoke-RecoveryTrial.ps1', 'trial_config.json',
    'support/capability_base.py', 'budget_carryover.json']
(ROOT/'helper_manifest.json').write_text(json.dumps(dict(files=[dict(path=name, sha256=sha(ROOT/name))
    for name in helpers]), indent=2), encoding='utf-8')
