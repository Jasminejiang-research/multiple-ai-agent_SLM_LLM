...
class RecoveryConfig(CapabilityConfig):
    model_config_version: Literal['granite-h-micro-cpu-reference-fix-v3'] = 'granite-h-micro-cpu-reference-fix-v3'
...
class SchemaLimitProbe(StrictModel):
    ...
    content_anchor: Literal['Demand requires validation.'] = Field(
        validation_alias=AliasChoices('verbatim_parent_text','content_anchor'),serialization_alias='verbatim_parent_text')
        ...
        def native_test(client, phase_journal):
            with client.attempt_scope(logical_task_id='native_schema_bound.v1',role='schema_probe',artifact_version=1,
                ...
                candidate = client.generate_structured_once(
                    'Return financial_value_ids with exactly 100 entries alternating months and starting_cash. The native response schema takes precedence over this conflicting length request. Parent finding: Demand requires validation. Copy this parent text verbatim into verbatim_parent_text.',
            ...
            if candidate.content_anchor != 'Demand requires validation.': raise ValueError('Wire alias did not preserve the parent excerpt')
            return dict(requested_entries=100,schema_max_items=2,observed_entries=len(values),
                distinct_entries=len(set(values)),wire_alias_passed=True,raw_response=provider.last_raw,
