...
class GroundedClaim(StrictModel):
    ...
    source_quality: float = Field(ge=0, le=1, allow_inf_nan=False, description="Model-proposed normalized score from 0 to 1, not a rating out of 5.")
    source_recency: float | None = Field(ge=0, le=1, allow_inf_nan=False, description="Model-proposed normalized score from 0 to 1; null if unknown. Never a calendar year.")
    ...
    financial_value_ids: list[str] = Field(description="Only distinct frozen financial value IDs directly relevant to this claim; [] when no financial value is referenced.")
    premise: str
    artifact_version: int = Field(ge=1)

    @field_validator("financial_value_ids")
    @classmethod
    def unique_financial_references(cls, values):
        if len(values) != len(set(values)):
            raise ValueError("financial_value_ids must contain unique IDs; use [] for no related financial value")
        return values
