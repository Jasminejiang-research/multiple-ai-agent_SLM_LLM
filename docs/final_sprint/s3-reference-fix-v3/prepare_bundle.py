...
write('budget_carryover.json',dict(result_path=str(previous_result),sha256=digest(previous_result),
    basis='Cumulative usage includes original failed capability trial and reference-fix-v1/v2.'))
config=load(STAGE/'trial_config.json')
config['model_config_version']='granite-h-micro-cpu-reference-fix-v3'
write('trial_config.json',config)
authorization=load(STAGE/'authorization.json')
authorization['amendment']='Use an explicit parent-prose wire field label; preserve canonical content_anchor and strict exact-text validation. Same A-D workflow, model, budget and retry allowances.'
