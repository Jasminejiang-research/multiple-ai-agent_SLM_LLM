...
import pytest
from trial_runner import RecoveryConfig, TrialBudget, verified_carryover, SchemaLimitProbe
...
def test_native_alias_probe_keeps_exact_text_and_array_limit():
    response=SchemaLimitProbe.model_validate(dict(financial_value_ids=['months','months'],verbatim_parent_text='Demand requires validation.'))
    assert response.content_anchor=='Demand requires validation.'
    assert 'verbatim_parent_text' in SchemaLimitProbe.model_json_schema()['properties']
    assert 'content_anchor' in response.model_dump()
