...
PRIOR=REPO/'docs/final_sprint/s3-90m-fix-v1'
...
for entry in files:
    ...
    if target.exists(): raise RuntimeError('Existing staging source; preserve')
    shutil.copy2(REPO/entry['path'],target)
(ROOT/'source_inventory.json').write_text(json.dumps(files,indent=2),encoding='utf-8')
(ROOT/'validation').mkdir(exist_ok=True)
print('Copied',len(files),'source/input files; no running source changed.')
