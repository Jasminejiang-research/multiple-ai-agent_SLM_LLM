...
from workflow.compact_protocol import (
    CONFIG_VERSION, GEMINI_8192_DUAL_LINEAGE_CONFIG_VERSION, MVP30_SECONDS,
    VALID_PLAN_SECONDS,
)
...
COMMON_FIELDS = ("max_requests", "max_total_tokens", "max_output_tokens", "max_prompt_chars",
                 ...
                 "temperature", "thinking_budget", "context_tokens", "protocol_version", "mvp_seconds",
                 "config_version")
...
AUTHORITY_NAMES = ("sprint_plan_.md","@ai_agent_evaluation_metrics_spec_v2_fast_generation.md","@multiple_ai_agent_optimization.md")
LLM_ONLY_BRANCH = "llm_only_after_D_no_go"
D_POLICY_STATUS = "not_run_preflight_no_go"
LLM_ONLY_CONDITIONS = ("A", "B", "C")


def verify_smoke_budget_epoch_authorization(authorization):
    """Verify the external user-authorization record; a frozen copy alone cannot reset spend."""
    if authorization is None:
        return
    if not isinstance(authorization, dict):
        raise ValueError("invalid smoke epoch authorization")
    ref = authorization.get("authorization_record", {})
    path = Path(ref.get("path", ""))
    if not path.is_file() or file_hash(path) != ref.get("sha256"):
        raise ValueError("smoke epoch authorization file missing or changed")
    record = read_json(path)
    required = ("version", "approved", "starts_at_experiment_version",
        "preserve_lifetime_history", "unchanged_hard_limits")
    if (any(record.get(key) != authorization.get(key) for key in required) or
            record.get("approved") is not True or
            not isinstance(record.get("user_message_verbatim"), str) or
            not record["user_message_verbatim"].strip()):
        raise ValueError("smoke epoch authorization does not match explicit user record")
...
def create_experiment(directory, *, case_root=ROOT/"docs/final_sprint/s0-v1", seed=20260906,
    ...
    plan = dict(version="eight-slot-s6-compact-v1", experiment_id=uuid4().hex, randomization_seed=seed,
    ...
    write_json(directory/"freeze.json", dict(status="pending", plan_sha256=canonical_hash(plan),
        approvals=dict(cases="pending", common_budget="deferred", D_preflight="deferred",
            gemini_quota_and_spend="pending", S1="passed", S2="passed", S4="passed", S6="pending",
...
def _result_state(result, resource_failure=None):
    ...
    return dict(execution=execution, terminal_contract=result.get("terminal_contract_state",
        ...
        mvp30=result.get("mvp30_state", "not_reached"),
        valid_plan60=result.get("valid_plan60_state", "not_reached"),
        artifact_class=result.get("artifact_class", "partial_or_failed"))
...
def status(directory):
    ...
    rows = {r["planned_id"]: dict(**r, execution="not_run", terminal_contract="not_checked",
        ...
        failure_reason=None, workflow_status=None, mvp30="not_reached", valid_plan60="not_reached",
        artifact_class="not_run") for r in plan["rows"]}
    ...
    for event in events:
        ...
        elif event["kind"] == "policy_not_run":
            if (row["condition"] != "D" or row["execution"] != "not_run" or
                    event.get("state") != D_POLICY_STATUS):
                raise ValueError("only an untouched D slot can receive the approved no-go policy state")
            row.update(execution=D_POLICY_STATUS, terminal_contract="not_checked",
                scorable_output="not_available", needs_human_review=False,
                failure_reason="non-formal D preflight no_go; formal Granite intentionally not run",
                workflow_status=D_POLICY_STATUS, artifact_class=D_POLICY_STATUS)
    ...
    return dict(experiment_id=plan["experiment_id"], kind=plan["kind"], rows=list(rows.values()),
        ...
        formal_runs_started=sum(r["run_id"] is not None for r in rows.values()) if plan["kind"] == "formal_plan" else 0)
...
def check_freeze(directory, *, root=ROOT):
    ...
    branch = freeze.get("execution_branch")
    llm_only = branch == LLM_ONLY_BRANCH
    for key in ("cases", "common_budget", "D_preflight", "gemini_quota_and_spend", "S1", "S2", "S4", "S6", "formal_execution"):
        accepted = (("approved_no_go_omit_D",) if key == "D_preflight" and llm_only else
            ("passed","approved") if key in ("S1","S2","S4","S6") else ("approved",))
    ...
    configs = {}
    try:
        expected_arms = LLM_ONLY_CONDITIONS if llm_only else tuple("ABCD")
        if set(freeze["configs"]) != set(expected_arms):
            raise ValueError("configuration arm set differs from the authorized execution branch")
        configs = {arm: ReviewRunConfig.model_validate(freeze["configs"][arm]) for arm in expected_arms}
        ...
        for arm, cfg in configs.items():
            ...
            expected_config_version = GEMINI_8192_DUAL_LINEAGE_CONFIG_VERSION if llm_only else CONFIG_VERSION
            expected_output_tokens = 8192 if llm_only else 1800
            if (cfg.protocol_version != WIRE_VERSION or cfg.config_version != expected_config_version or
                    cfg.run_seconds != VALID_PLAN_SECONDS or cfg.mvp_seconds != MVP30_SECONDS or
                    cfg.max_output_tokens != expected_output_tokens or cfg.max_prompt_chars > 26000 or
                    (llm_only and cfg.thinking_budget != 1024)):
                errors.append("S6 compact protocol/common 30/60 limits are not frozen")
        ...
        if llm_only and (any((cfg.max_requests, cfg.max_total_tokens, cfg.run_seconds) != (18, 160000, 3600)
                for cfg in configs.values()) or freeze.get("comparison_policy") != {
                    "reported": ["B-A", "C-B"], "unavailable": {"C-D": "D_preflight_no_go"},
                    "success_subset_substitution": False}):
            errors.append("LLM-only ABC budgets/comparison policy differ from the approved scope")
    ...
    # The ordinary branch requires D go. The explicitly authorized LLM-only branch
    # instead requires immutable no-go evidence plus both D rows permanently sealed.
    try:
        report_ref = freeze["preflight_reports"]["D"]
        report_path = Path(report_ref["path"])
        report = read_json(report_path)
        if file_hash(report_path) != report_ref["sha256"]:
            raise ValueError("D evidence hash changed")
        if llm_only:
            decision = freeze["approval_records"]["D_preflight"]
            decision_path = Path(decision["path"])
            decision_record = read_json(decision_path)
            if (file_hash(decision_path) != decision["sha256"] or report.get("status") != "no_go" or
                    report.get("record_label") != "nonformal_engineering_preflight_failure" or
                    report.get("formal_experiment_result") is not False or
                    report.get("c_minus_d_comparison_eligible") is not False or
                    decision_record.get("execution_branch") != LLM_ONLY_BRANCH or
                    decision_record.get("approved") is not True):
                raise ValueError("approved D no-go omission evidence missing")
            d_rows = [r for r in status(directory)["rows"] if r["condition"] == "D"]
            if len(d_rows) != 2 or any(r["execution"] != D_POLICY_STATUS or r["run_id"] is not None for r in d_rows):
                raise ValueError("both D slots must be permanently sealed without dispatch")
            demo = freeze.get("excluded_nonformal_artifacts", {}).get("best_effort_demo", {})
            if demo.get("formal_eligible") is not False or demo.get("c_minus_d_comparison_eligible") is not False:
                raise ValueError("best-effort demo exclusion missing")
        else:
            if report["status"] != "go" or report["requested_phase"] != "all":
        ...
        errors.append("approved D no-go LLM-only branch evidence pending" if llm_only else
            "complete D real preflight and matching common config pending")
    for arm in "ABC":
        try:
    ...
    if llm_only:
        limits = freeze.get("cost_limits_usd", {})
        smoke = freeze.get("smoke_actual", {})
        prior = freeze.get("prior_nonformal_spend", {})
        try:
            prior_records = prior["records"]
            recomputed = {"requests": 0, "total_tokens": 0, "cost_usd": 0.0}
            required_baseline = {f"s5-llm-only-v{number}" for number in range(1, 7)}
            seen = set()
            for ref in prior_records:
                path = Path(ref["path"])
                summary = read_json(path)
                actual = summary["actual"]
                if (file_hash(path) != ref["sha256"] or
                        any(actual[key] != ref[key] for key in ("requests", "total_tokens", "cost_usd"))):
                    raise ValueError("prior smoke evidence changed")
                seen.add(path.parent.name)
                recomputed["requests"] += actual["requests"]
                recomputed["total_tokens"] += actual["total_tokens"]
                recomputed["cost_usd"] = round(recomputed["cost_usd"] + actual["cost_usd"], 9)
            prior_valid = (required_baseline <= seen and
                all(prior.get(key) == recomputed[key] for key in recomputed) and
                prior.get("requests", 0) >= 8 and prior.get("total_tokens", 0) >= 62281 and
                prior.get("cost_usd", 0) >= 0.1023283)
            ...
            budget_baseline = resolve_smoke_budget_baseline(prior,
                freeze.get("smoke_budget_epoch_authorization"))
            ...
            if freeze.get("smoke_budget_baseline", budget_baseline) != budget_baseline:
                raise ValueError("frozen budget baseline changed")
            active_prior = budget_baseline["active_prior"]
        except (KeyError, OSError, TypeError, ValueError):
            prior_valid = False
            active_prior = prior
        if (freeze.get("total_request_limit") != 108 or freeze.get("total_token_limit") != 960000 or
                limits != {"smoke": 1.2, "formal": 2.4, "combined": 3.6,
                    "conservative_rate_per_million_tokens": 2.5} or
                not prior_valid or
                type(smoke.get("requests")) is not int or active_prior.get("requests", 0) + smoke["requests"] > 54 or
                type(smoke.get("total_tokens")) is not int or active_prior.get("total_tokens", 0) + smoke["total_tokens"] > 480000 or
                not isinstance(smoke.get("cost_usd"), (int, float)) or active_prior.get("cost_usd", 0) + smoke["cost_usd"] > 1.2 or
                active_prior.get("cost_usd", 0) + smoke.get("cost_usd", 0) > limits.get("combined", 0)):
            errors.append("LLM-only aggregate request/token/cost limits are incomplete or exceeded")
...
class FormalPermit:
    ...
    def validate(self, context, config, run_id, planned_id):
        ...
        freeze = read_json(self.directory/"freeze.json")
        if freeze.get("execution_branch") == LLM_ONLY_BRANCH and row["condition"] == "D":
            raise ValueError("formal Granite dispatch is prohibited by the approved LLM-only branch")
        ...
        if config.model_dump(mode="json") != freeze["configs"][config.condition]:
        ...
        for row in snapshot["rows"]:
            ...
            if not synthetic:
                ...
                if any(p["usage_incomplete"] for p in prior) or sum(p["request_count"] for p in prior)+cfg["max_requests"] > freeze["total_request_limit"] or sum(p["total_tokens"] or 0 for p in prior)+cfg["max_total_tokens"] > freeze["total_token_limit"]:
                    ...
                    break
                if freeze.get("execution_branch") == LLM_ONLY_BRANCH:
                    rate = freeze["cost_limits_usd"]["conservative_rate_per_million_tokens"]
                    formal_reserved = (sum(p["total_tokens"] or 0 for p in prior) + cfg["max_total_tokens"]) * rate / 1_000_000
                    from evaluation.s5_smoke_budget import resolve_smoke_budget_baseline
                    baseline = resolve_smoke_budget_baseline(freeze.get("prior_nonformal_spend", {}),
                        freeze.get("smoke_budget_epoch_authorization"))
                    prior_spend = baseline["active_prior"]["cost_usd"]
                    combined_reserved = prior_spend + freeze["smoke_actual"]["cost_usd"] + formal_reserved
                    if (formal_reserved > freeze["cost_limits_usd"]["formal"] + 1e-12 or
                            combined_reserved > freeze["cost_limits_usd"]["combined"] + 1e-12):
                        append_event(directory/"schedule.jsonl", dict(kind="stop", reason="formal/combined conservative cost reservation"))
                        break
            run_id = uuid4().hex
...
def synthetic_executor(plan, row, run_id, run_dir, permit):
    ...
    config = ReviewRunConfig(condition=row["condition"], run_kind="debug", provider="mock",
        ...
        max_requests=32, max_total_tokens=20_000_000, max_output_tokens=1800,
        max_prompt_chars=26000, request_seconds=10, run_seconds=3600,
        protocol_version=WIRE_VERSION, mvp_seconds=MVP30_SECONDS, config_version=CONFIG_VERSION)
...
def real_executor(plan, row, run_id, run_dir, permit):
    ...
    if row["condition"] == "D":
        ...
        key, credential_source = resolve_gemini_credential(
            expected_source=freeze["model_metadata"]["A-C"]["credential_source_variable"])
        append_event(permit.directory/"schedule.jsonl", dict(kind="credential_selection",
            planned_id=row["planned_id"], variable_name=credential_source,
