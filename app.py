...
def _multi_agent_run_budget_settings() -> tuple[int, int]:
    """B has a separate explicit budget; A/legacy defaults are unchanged."""
    load_dotenv(ROOT_DIR / '.env')
    return (_positive_int_setting('LLM_MULTI_AGENT_RUN_MAX_REQUESTS', 24),
            _positive_int_setting('LLM_MULTI_AGENT_RUN_MAX_TOTAL_TOKENS', 420_000))
...
from workflow.product_acceptance import accept_a, render_a, quality_check, resolve_for_acceptance
...
def generate_proposal(client: LLMClient, user_idea: str) -> BusinessProposal:
    ...
    from workflow.product_finance import FINANCE_INSTRUCTION, validate_financial_prose
    from workflow.product_research import PROVENANCE_INSTRUCTION, validate_certainty
    from workflow.product_finance import strings
    from workflow.product_finance import (FinancialPlan, financial_plan_prompt,
        financial_plan_context, bind_financial_plan)
    from workflow.product_output_policy import language_instruction, validate_text_language
    plan = FinancialPlan.model_validate(client.generate_structured(
        financial_plan_prompt(user_idea), FinancialPlan, temperature=0.2))
    def validate_output(candidate):
        output_texts = list(strings(candidate.model_dump(mode='json', exclude={'product_acceptance'})))
        validate_text_language(' '.join(output_texts), 'en')
        bind_financial_plan(candidate, plan)
        for text in output_texts:
            validate_certainty(text)
    result = client.generate_structured(
        user_idea + FINANCE_INSTRUCTION + PROVENANCE_INSTRUCTION + financial_plan_context(plan)
    ...
    quality_check(validate_output, result)
    return result
...
def proposal_to_markdown(proposal: BusinessProposal) -> str:
    ...
    from workflow.product_finance import validate_financial_prose, resolve_tokens, render_ledger
    quality_check(validate_financial_prose, proposal.model_dump(exclude={'product_acceptance'}))
    from workflow.product_research import validate_certainty
    from workflow.product_finance import strings
    output_texts = list(strings(proposal.model_dump(mode='json', exclude={'product_acceptance'})))
    from workflow.product_output_policy import validate_text_language
    validate_text_language(' '.join(output_texts), 'en')
    for text in output_texts:
        quality_check(validate_certainty, text)
    lines: list[str] = [
    ...
    lines.extend(
        [
            ...
            ("| Runway | " + "{{fin:" + proposal.financial_model.scenarios[0].scenario_id + ".runway_months}} months |"
             if proposal.financial_model else "| Runway | Needs validation (no cash-flow assumption ledger) |"),
    ...
    markdown = resolve_for_acceptance("\n".join(lines),proposal.financial_model)+render_ledger(proposal.financial_model)
    from workflow.product_output_policy import validate_english_document
    validate_english_document(markdown)
    return markdown


def proposal_audit_to_markdown(proposal: BusinessProposal) -> str:
    """Render A's execution/acceptance metadata as a separate audit report."""
    from workflow.product_acceptance import acceptance_notice
    from workflow.product_output_policy import validate_english_document
    generated = datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')
    body = acceptance_notice(proposal.product_acceptance)
    markdown = '\n'.join([
        '# A Product Audit Report', '',
        f'Generated by Open Proposal Agent (Phase 1) at {generated}.', '',
        body.rstrip(), '',
...
def proposal_audit_path(proposal_path: Path) -> Path:
    """Return the deterministic sibling path for A's audit report."""
    return proposal_path.with_name(proposal_path.stem + '_audit' + proposal_path.suffix)
...
def save_proposal_markdown(proposal: BusinessProposal, output_dir: Path | None = None) -> Path:
    ...
    proposal_audit_path(output_path).write_text(proposal_audit_to_markdown(proposal), encoding="utf-8")
    return output_path
...
def build_run_detail(run: Any) -> RunDetail:
    ...
    for node_output in sorted(
        ...
        snapshot = node_output.output_snapshot or {}
        if snapshot.get("event_version") and "kind" in snapshot:
            from evaluation.s4_ui import event_step
            step, status, body = event_step(snapshot)
            latest_by_step[step] = NodeOutputPreview(step=step, status=status,
                output_preview=_json_preview(body, max_chars=4000))
            continue
...
def run_workflow_pipeline(user_brief: dict[str, str]) -> WorkflowPipelineResult:
    ...
    return WorkflowPipelineResult(
        ...
        sources=[],
        web_sources=[],
        audit_path=Path(result['audit_path']) if result.get('audit_path') else None,
        audit_markdown=result.get('audit_markdown'),
    )
...
def run_multi_agent_pipeline(user_brief: dict[str, str]) -> WorkflowPipelineResult:
    ...
    max_requests, max_total_tokens = _multi_agent_run_budget_settings()
    with run_budget(
        max_requests=max_requests,
        max_total_tokens=max_total_tokens,
    ) as budget:
...
def run_streamlit_app() -> None:
    ...
    with st.sidebar:
        ...
        run_mode = st.radio(
            "Run Mode",
            ["Baseline", "Workflow", "Multi-Agent", "S4 Experiment Review"],
            index=0,
            help=(
        ...
        generate_clicked = st.button("Generate Proposal", type="primary", use_container_width=True,
                                     disabled=run_mode == "S4 Experiment Review")
        _render_recent_runs_sidebar(st)

    if run_mode == "S4 Experiment Review":
        from evaluation.s4_ui import render_experiment_panel
        render_experiment_panel(st)
        return
        ...
        if missing:
            ...
            try:
                if run_mode in {"Workflow", "Multi-Agent"}:
                    ...
                    if workflow_result.audit_markdown and workflow_result.audit_path:
                        st.session_state["proposal_audit_markdown"] = workflow_result.audit_markdown
                        st.session_state["proposal_audit_path"] = str(workflow_result.audit_path)
                        st.session_state["proposal_audit_name"] = workflow_result.audit_path.name
                    else:
                        st.session_state.pop("proposal_audit_markdown", None)
                        st.session_state.pop("proposal_audit_path", None)
                        st.session_state.pop("proposal_audit_name", None)
                    ...
                    audit_path = proposal_audit_path(output_path)
                    st.session_state["proposal_audit_markdown"] = audit_path.read_text(encoding='utf-8')
                    st.session_state["proposal_audit_path"] = str(audit_path)
                    st.session_state["proposal_audit_name"] = audit_path.name
    ...
    if "markdown" in st.session_state:
        ...
        if "proposal_audit_markdown" in st.session_state:
            st.download_button(
                label="Download Audit Report",
                data=st.session_state["proposal_audit_markdown"],
                file_name=st.session_state.get("proposal_audit_name", "business_proposal_audit.md"),
                mime="text/markdown",
            )
            st.caption(f"Audit report saved to `{st.session_state['proposal_audit_path']}`")
        st.divider()
