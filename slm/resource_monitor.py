...
from slm.windows_memory import read_pagefile_used_bytes
...
class WindowsResourceProbe:
    ...
    def __call__(self):
        ...
        try:
            ...
            result["pagefile_used_bytes"] = read_pagefile_used_bytes()
        except Exception as exc:
...
class ResourceMonitor:
    def __init__(self, probe, journal, cancel, *, interval=5, abort=None):
        ...
        self.maximum_allowed_sample_gap_seconds = max(3 * interval, 15)
    ...
    def _record(self, sample):
        ...
        gap = sample["elapsed_seconds"] - self.samples[-1]["elapsed_seconds"] if self.samples else 0
        if gap > self.maximum_allowed_sample_gap_seconds:
            self.observation_gaps.append(dict(gap_seconds=gap,
                threshold_seconds=self.maximum_allowed_sample_gap_seconds,
                previous_elapsed_seconds=self.samples[-1]["elapsed_seconds"],
                current_elapsed_seconds=sample["elapsed_seconds"]))
            # A healthy reading after a long unobserved interval cannot certify
            # the missing period. Preserve earlier failure causes if present.
            self.gate.failure = self.gate.failure or "resource_observation_gap"
        ...
        if failure:
            self.journal.emit("resource_stop", dict(reason=failure, baseline=self.baseline, last_sample=sample,
                observation_gap_seconds=gap, maximum_allowed_sample_gap_seconds=self.maximum_allowed_sample_gap_seconds))
            self.cancel.set()
            if self.abort:
                self.abort()
    ...
    def close(self):
        ...
        summary = dict(status="failed" if self.gate.failure else "passed", reason=self.gate.failure,
            ...
            maximum_allowed_sample_gap_seconds=self.maximum_allowed_sample_gap_seconds,
            observation_gap_count=len(self.observation_gaps), observation_gaps=self.observation_gaps,
