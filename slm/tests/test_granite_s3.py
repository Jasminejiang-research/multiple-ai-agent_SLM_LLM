...
class FakeOwner:
    ...
    def stop(self):
        self.proof={"status":"owned_process_tree_terminated", "lease_released":False, "synthetic":True}
        return self.proof


def make_runner(tmp_path, monkeypatch, config, context, *, before=None, probe=None, resume_from=None):
    import slm.granite_preflight as module
    monkeypatch.setattr(module,"load_context_tokenizer",lambda *args:object())
    monkeypatch.setattr(module,"inspect_ollama",lambda config:dict(
        loaded_models=[dict(context_length=32768, size_vram=0)]))
    ...
    def factory(config, *, model_digest, context_tokenizer=None):
        ...
        def invoke(request):
            if request.schema is ProbeAck and before is None:
                ...
                count = 12000 if config.protocol_version == WIRE_VERSION else 31000
                return PhysicalResponse('{"ok":true}',dict(prompt_eval_count=count,
                    context_probe_expected_tokens=count,context_probe_token_count_delta=0),count,4,count+4)
    ...
    runner=GranitePreflight(config=config, context=context, output_dir=tmp_path/"run", owner_path="unused",
        ...
        awake_factory=lambda **kwargs:nullcontext(), resume_from=resume_from)
    runner.lease_dir=tmp_path/"leases"
    return runner, providers


def test_compact_preflight_warmup_and_context_use_bounded_probe_aliases(tmp_path, monkeypatch, context):
    import slm.granite_preflight as module
    cfg = GraniteConfig.model_validate_json(
        (ROOT / "slm/configs/granite_preflight_compact_s6.json").read_text())
    (tmp_path/"warmup").mkdir()
    warmup, providers = make_runner(tmp_path/"warmup", monkeypatch, cfg, context)
    result = warmup.run("warmup")
    assert result["status"] == "phase_passed" and len(providers[0].calls) == 1
    assert providers[0].calls[0].max_output_tokens == 1800

    (tmp_path/"context").mkdir()
    context_runner, providers = make_runner(tmp_path/"context", monkeypatch, cfg, context)
    monkeypatch.setattr(module, "inspect_ollama", lambda _: {
        "loaded_models": [{"context_length": 32768, "size_vram": 0}]})
    result = context_runner.run("context")
    phase = result["phases"]["context"]
    assert result["status"] == "phase_passed" and phase["compact_prompt_cap_verified"]
    request = providers[0].calls[0]
    assert len(request.prompt) + len(request.system_instruction or "") <= 26000


def test_compact_preflight_components_use_compact_protocol(tmp_path, monkeypatch, context):
    cfg = GraniteConfig.model_validate_json(
        (ROOT / "slm/configs/granite_preflight_compact_s6.json").read_text())
    runner, providers = make_runner(tmp_path, monkeypatch, cfg, context)
    result = runner.run("components")
    assert result["status"] == "phase_passed", result
    assert all(probe["status"] == "passed" for probe in result["phases"]["components"]["probes"].values())
    assert sum(len(provider.calls) for provider in providers) == 4
    assert result["cumulative_budget"]["request_count"] == 4


def test_compact_full_preflight_shares_one_budget_across_phases(tmp_path, monkeypatch, context):
    cfg = GraniteConfig.model_validate_json(
        (ROOT / "slm/configs/granite_preflight_compact_s6.json").read_text())
    runner, providers = make_runner(tmp_path, monkeypatch, cfg, context)
    result = runner.run()
    calls = sum(len(provider.calls) for provider in providers)
    assert result["status"] == "go", result
    assert result["model_calls"] == calls == result["cumulative_budget"]["request_count"]
    assert calls <= cfg.max_requests and result["cumulative_budget"]["total_tokens"] <= cfg.max_total_tokens
    assert (result["cumulative_budget"]["prompt_tokens"] + result["cumulative_budget"]["output_tokens"]
        == result["cumulative_budget"]["total_tokens"])


def test_compact_resume_reuses_passed_probes_and_restores_cumulative_budget(tmp_path, monkeypatch, context):
    cfg = GraniteConfig.model_validate_json(
        (ROOT / "slm/configs/granite_preflight_compact_s6.json").read_text())
    prior_root = tmp_path / "prior"
    prior_root.mkdir()
    prior, _ = make_runner(prior_root, monkeypatch, cfg, context)
    def fail_before_component_dispatch():
        prior.phases["components"] = {"status": "failed", "reason": "synthetic_pre_dispatch_failure"}
        return False
    prior.components = fail_before_component_dispatch
    failed = prior.run()
    assert failed["status"] == "no_go" and failed["model_calls"] == 2
    result_path = tmp_path / "prior" / "run" / "result.json"

    resumed_root = tmp_path / "resumed"
    resumed_root.mkdir()
    resumed, providers = make_runner(resumed_root, monkeypatch, cfg, context,
        resume_from=result_path)
    result = resumed.run("resume")
    new_calls = sum(len(provider.calls) for provider in providers)
    assert result["status"] == "go" and result["requested_phase"] == "all", result
    assert result["model_calls"] == 2 + new_calls
    assert result["cumulative_budget"]["request_count"] == result["model_calls"]
    assert result["resume"]["prior_model_calls"] == 2
...
def test_all_phases_and_complete_common_graph_offline(tmp_path, monkeypatch, config, context):
    ...
    cfg=config.model_copy(update=dict(max_requests=30,max_total_tokens=20_000_000,max_prompt_chars=1_000_000))
    runner, providers=make_runner(tmp_path,monkeypatch,cfg,context)
    result=runner.run()
    assert result["status"]=="go",result
...
def test_warmup_timeout_skips_dependents_and_quarantines_lease(tmp_path,monkeypatch,config,context):
    ...
    assert result["server_stop"]["synthetic"] is True
