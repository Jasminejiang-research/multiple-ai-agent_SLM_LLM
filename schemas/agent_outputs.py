...
from workflow.product_finance import ProductFinancialModel
...
class ResearchFinding(BaseModel):
    ...
    model_config = ConfigDict(str_strip_whitespace=True, extra="forbid")
    evidence_status: Literal['sourced_fact','assumption','needs_validation'] = 'needs_validation'
...
class ResearchAnalysis(BaseModel):
    ...
    model_config = ConfigDict(str_strip_whitespace=True, extra="forbid")
    evidence_notice: str = 'Research findings are hypotheses unless their explicit evidence status and sources pass validation.'
...
class FinanceAssumptions(BaseModel):
    financial_model: ProductFinancialModel | None = None
...
class WriterInput(BaseModel):
    ...
    user_brief: dict[str, Any] = Field(default_factory=dict)
    ...
    def derive_evidence_mode(self) -> "WriterInput":
        ...
        if self.user_brief:
            ...
            catalog=evidence_catalog(self.user_brief,self.web_sources,self.evidence_chunks)
            self.low_confidence_required=not any(item['role']=='eligible_external' for item in catalog)
        return self
