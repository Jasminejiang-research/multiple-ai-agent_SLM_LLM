...
from workflow.product_output_policy import language_instruction, validate_product_batch
from workflow.product_checkpoint import current_store
...
def build_writer_prompt(input_data: dict[str, Any] | WriterInput) -> str:
    ...
    return (
        ...
        "# Writer Input JSON\n\n"
        f"```json\n{json.dumps(writer_input.model_dump(mode='json'),ensure_ascii=False,separators=(',', ': '))}\n```"
        + language_instruction(writer_input.output_language)
        + PROVENANCE_INSTRUCTION
        + json.dumps({'evidence_catalog':catalog},ensure_ascii=False)
        + FINANCE_INSTRUCTION
        + render_ledger(writer_input.finance_assumptions.financial_model)
    )
...
class WriterAgent(BaseAgent):
    ...
    def _run(self, input_data: Any) -> ProposalDraft:
        ...
        from workflow.product_acceptance import current_acceptance, quality_check
        ...
        source_roles = ({item['source_id']:item['role'] for item in evidence_catalog(
            writer_input.user_brief,writer_input.web_sources,writer_input.evidence_chunks)}
            if writer_input.user_brief else None)

        def validate_writer_output(candidate: ProposalDraft) -> None:
            """Aggregate unknown IDs and missing markers in one correction."""
            validate_product_batch(candidate, language=writer_input.output_language,
                                   allowed=allowed_source_ids, citations=source_roles is not None,source_roles=source_roles)
            validate_proposal_citations(
            ...
            validate_financial_prose(candidate,writer_input.finance_assumptions.financial_model)
        ...
        if callable(batch_generator):
            ...
            if store is not None:
                store.bind(writer_input, prompt)
            batch_outputs = []
            for batch_number, (section_fields, batch_model) in enumerate(
                ...
                validate_batch=product_batch_validator(language=writer_input.output_language,
                    allowed=allowed_source_ids,source_roles=source_roles,
                    financial_model=writer_input.finance_assumptions.financial_model)
                from workflow.product_repair_context import writer_repair_context
                validate_batch.repair_context=lambda fields,original: writer_repair_context(writer_input,fields,original)

                cached = store.get(batch_number, batch_model) if store else None
                if cached is not None:
                    quality_check(validate_batch, cached)
                    batch_outputs.append(cached)
                    continue
                raw_batch = batch_generator(batch_prompt, batch_model, validate_batch)
                batch = batch_model.model_validate_json(raw_batch)
                quality_check(validate_batch, batch)  # Retain residual checks for custom adapters too.
                if store is not None:
                    store.save(batch_number, batch)
            ...
            if failures and current_acceptance() is None:
                ...
                for section_name, section_failures in failures_by_section.items():
                    ...
                    patch_prompt += language_instruction(writer_input.output_language)
                    ...
                    def validate_writer_patch(
                        ...
                        *,
                        ...
                        validate_product_batch(ProposalDraft.model_validate(candidate_data),
                            language=writer_input.output_language,allowed=allowed_source_ids,citations=False)
        ...
        quality_check(validate_writer_output, proposal)
        return proposal
