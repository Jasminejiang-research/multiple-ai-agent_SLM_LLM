...
def build_research_prompt(
    ...
    return (
        ...
        + json.dumps({'evidence_catalog':evidence_catalog(user_brief,web_sources or [],[])},ensure_ascii=False)
    )
...
class ResearchAgent(BaseAgent):
    ...
    def _market_query(user_brief: dict[str, Any]) -> str:
        ...
        from workflow.product_research import compact_query
    ...
    def collect_web_sources(
        ...
        """
        from workflow.evidence_policy import require_live_evidence_access
        require_live_evidence_access()
        from workflow.product_research import plan_queries
        llm = self._llm_client
        if llm is None:
            llm = create_default_research_llm()
            self._llm_client = llm
        self._research_plan = plan_queries(user_brief,llm)
        search_requests = (
            ("Market Research Agent", self._market_query(user_brief)),
            ("Competitor Agent", self._competitor_query(user_brief)),
        )
        sources: list[SourceRecord] = []
        if self._research_plan is not None:
            search_requests = (
                ('Market Research Agent',self._research_plan.market_query),
                ('Competitor Agent',self._research_plan.service_and_regulatory_query),
            )
    ...
    def _run(self, input_data: Any) -> ResearchAnalysis:
        ...
        if input_data.get("execution_mode") == "formal_frozen":
            raise ValueError("Use ContractContext/ContractGenerator for frozen Research; legacy Research is not an experimental path")
        ...
        catalog=evidence_catalog(prompt_brief,web_sources,[])
        prompt += ('\nResearchFinding evidence_status is sourced_fact, assumption or needs_validation. '
                   'source_ids must be exact IDs from the evidence catalog. User input is not a source. '
                   'Unverified findings and rationales must be qualified, with low confidence.\n')
        def validate(candidate): validate_research_provenance(candidate,catalog)
        generator=getattr(llm_client,'generate_json_validated',None)
        raw_output=generator(prompt,validate) if callable(generator) else llm_client.generate_json(prompt)
        result=parse_research_analysis(raw_output)
        from workflow.product_acceptance import quality_check
        quality_check(validate, result)
        return result
