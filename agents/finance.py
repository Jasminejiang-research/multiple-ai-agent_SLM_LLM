...
def build_finance_prompt(input_data: dict[str, Any]) -> str:
    ...
    from workflow.product_finance import FINANCE_INSTRUCTION
    from workflow.product_research import PROVENANCE_INSTRUCTION
...
class FinanceAgent(BaseAgent):
    ...
    def _run(self, input_data: Any) -> FinanceAssumptions:
        ...
        planner=getattr(llm_client,'generate_json_for_schema',None)
        # Legacy injected adapters may only return a completed packet. Production
        # always supports the shared structured adapter and runs the planning stage.
        plan=(FinancialPlan.model_validate_json(planner(financial_plan_prompt(input_data),FinancialPlan))
              if callable(planner) else None)
        if plan is not None:
            prompt += financial_plan_context(plan)
        def validate_output(candidate):
            if plan is not None: bind_financial_plan(candidate,plan)
            else: validate_financial_prose(candidate)
        generator=getattr(llm_client,'generate_json_validated',None)
        raw_output=(generator(prompt,validate_output) if callable(generator)
                    else llm_client.generate_json(prompt))
        result=parse_finance_assumptions(raw_output)
