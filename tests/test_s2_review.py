...
def test_score_is_computed_on_fixed_scale(role, score, expected):
    ...
    # S6 freezes the score as diagnostic-only; only high/critical issues gate.
    assert not report.revision_required
...
@pytest.mark.parametrize("role", METRICS)
@pytest.mark.parametrize("severity,score", [("high", 4), ("critical", 4)])
...
def test_mixed_score_is_not_rounded_before_gate():
    ...
    assert report.overall_score == 6.875 and not report.revision_required


def test_low_score_without_high_issue_is_diagnostic_only(tmp_path):
    workflow, provider = setup_run(tmp_path, condition="C", fail_roles=["research"],
    ...
    assert not any(call["task_purpose"] == "revision" for call in calls(workflow))
...
def test_gemini_adapter_makes_one_sdk_call_with_remaining_timeout_and_raw_usage():
    ...
    assert received[0]["config"].thinking_config.thinking_budget == 1024
    assert received[0]["config"].thinking_config.include_thoughts is False
