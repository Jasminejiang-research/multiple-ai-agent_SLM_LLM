...
class FakeClient:
    ...
    def generate_structured_once(self, prompt, schema, **kwargs):
        ...
        role = next((role for model, role in roles.items() if issubclass(canonical, model)), None)
        if role is not None:
            payload = component_payload(self.context, role)
        else:
            full = proposal_payload(self.context)
            payload = {key: full[key] for key in canonical.model_fields}
        from evaluation.two_stage_fixtures import synthetic_stage_payload
        payload = synthetic_stage_payload(schema, payload)
...
def test_common_generation_path_and_first_research_evidence(condition, tmp_path):
    ...
    assert len(client.calls) == (8 if condition == "A" else 14)  # S1 primitives, no S2 Critic graph.
    assert [len(fields) for fields in PROPOSAL_SECTION_BATCHES] == [4, 3, 3, 3]
    for index, (_, schema, _) in enumerate(client.calls[-8:]):
...
def test_first_schema_failure_is_never_rewritten_after_repair(context):
    ...
    assert [a.passed for a in task.attempts] == [False, True, True]
    assert [a.purpose for a in task.attempts] == ["generate", "structure_repair", "generate"]
...
def test_cross_batch_collision_fails_terminal_without_extra_repair():
    ...
    def inconsistent(prompt, schema, **kwargs):
        candidate = original(prompt, schema, **kwargs)
        if getattr(schema, "__canonical_schema__", None) is CONTRACT_BATCH_MODELS[-1] and schema.__generation_stage__ == "grounding":
