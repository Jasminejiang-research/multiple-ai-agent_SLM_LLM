...
def _upstream_evidence_catalog(state):
    from workflow.product_research import evidence_catalog
    return evidence_catalog(state.get('user_brief') or {},
    ...
    analysis = agent.run(
        {
            ...
            "evidence_catalog": _upstream_evidence_catalog(state),
        }
    )
    return {
...
def writer_agent_node(
    ...
    *,
    ...
    checkpoint = None
    if checkpoint_root is not None and state.get("run_id"):
        checkpoint = checkpoint_path(checkpoint_root, state["run_id"])
        with writer_checkpoint(checkpoint, resume=resume):
            proposal = agent.run(writer_input)
    elif resume:
        raise ValueError("Writer resume requires checkpoint_root and run_id")
    else:
        proposal = agent.run(writer_input)
    return {
        "output_language": language,
        "writer_checkpoint_path": str(checkpoint) if checkpoint else None,
...
def critic_agent_node(
    ...
    *,
    ...
    from workflow.product_finance import resolve_payload, render_ledger, validate_financial_prose
    model=(state.get('finance_assumptions') or {}).get('financial_model')
    from workflow.product_acceptance import quality_check, current_acceptance
    quality_check(validate_financial_prose, proposal_draft, model)
    try:
        review_copy=resolve_payload(proposal_draft,model)
    except ValueError as exc:
        if current_acceptance() is None:
            raise
        current_acceptance().flag(exc)
        from copy import deepcopy
        review_copy=deepcopy(proposal_draft)
    if model is not None:
        review_copy['financial_assumptions']['content']+=render_ledger(model)
    critique = agent.run(
        {
            "proposal_draft": review_copy,
