...
from workflow.logging import (
    ...
    with_workflow_logging as strict_workflow_logging,
)
...
from workflow.review_graph import build_review_workflow  # Explicit versioned S2 entry; legacy factory below is unchanged.


def with_workflow_logging(step_name, node, session_factory):
    from workflow.product_acceptance import product_node
    return strict_workflow_logging(step_name, product_node(step_name, node), session_factory)
...
def build_multi_agent_workflow_graph(
    *,
    ...
    writer_checkpoint_root: Path | None = None,
    resume_writer: bool = False,
) -> Any:
    ...
    checkpoint_root = writer_checkpoint_root or (
        (output_dir or Path(__file__).resolve().parents[1] / "outputs") / "writer_checkpoints"
    )
    graph = StateGraph(WorkflowState)

    def validate_resume(state):
        from schemas.agent_outputs import WriterInput
        from workflow.product_output_policy import resolve_output_language
        from workflow.run_budget import get_active_run_budget
        if get_active_run_budget() is None:
            raise ValueError("Writer resume requires an active caller-owned cumulative run budget")
        if not state.get("run_id"):
            raise ValueError("Explicit Writer resume requires the original run_id")
        WriterInput.model_validate({
            key: state[key] for key in ("research_analysis", "strategy_analysis", "finance_assumptions",
                                       "web_sources", "evidence_chunks")
        })
        return {"output_language": resolve_output_language(state.get("user_brief") or {})}

    graph.add_node("resume_validate", validate_resume)
    graph.add_edge("resume_validate", "writer")
    graph.add_node(
        "validator",
    ...
    graph.add_node(
        ...
        _logged_agent_node(
            ...
            node=lambda state: writer_agent_node(state, agent=writer,
                checkpoint_root=checkpoint_root, resume=resume_writer),
    ...
    graph.add_conditional_edges(
        "validator",
        lambda state: ("missing_info" if state.get("missing_info") else
                       "resume_validate" if resume_writer else "supervisor"),
        {"missing_info": END, "supervisor": "supervisor", "resume_validate": "resume_validate"},
    )
    ...
    def seal_writer_recovery(state):
        # Write before any Critic call. Writer-only usage snapshots cannot cover
        # downstream failures or authorize replay of an already completed run.
        from workflow.product_checkpoint import write_once
        path = state.get("writer_checkpoint_path")
        if path:
            write_once(Path(path) / "downstream-started.json",
                       {"run_id": state["run_id"], "next_step": "critic"})
        return {}

    graph.add_node("seal_writer_recovery", seal_writer_recovery)
    graph.add_edge("writer", "seal_writer_recovery")
    graph.add_edge("seal_writer_recovery", "critic")
