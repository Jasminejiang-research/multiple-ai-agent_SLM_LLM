...
def input_validator_node(state: WorkflowState) -> WorkflowState:
    ...
    """
    if state.get("execution_mode") == "formal_frozen":
        raise ValueError("Legacy graph is not a formal A-D workflow; use the versioned contract entry")
...
def export_node(
    ...
    """
    from workflow.product_output_policy import resolve_output_language, validate_product_batch as strict_validate_product_batch
    language = resolve_output_language(state.get("user_brief") or {})
    from workflow.product_finance import validate_financial_prose as strict_validate_financial_prose, render_ledger
    from workflow.product_acceptance import quality_check, current_acceptance, resolve_for_acceptance
    from workflow.product_audit import clean_business_content, render_b_audit_report, render_formal_b_proposal
    def validate_product_batch(*args, **kwargs):
        return quality_check(strict_validate_product_batch, *args, **kwargs)
    def validate_financial_prose(*args, **kwargs):
        return quality_check(strict_validate_financial_prose, *args, **kwargs)
    financial_model=(state.get('finance_assumptions') or {}).get('financial_model')
    def validate_export_evidence(proposal):
        ...
        catalog=evidence_catalog(state['user_brief'],
            [SourceRecord.model_validate(v) for v in state.get('web_sources',[])],
            [EvidenceChunk.model_validate(v) for v in state.get('evidence_chunks',[])])
        roles={item['source_id']:item['role'] for item in catalog}
        validate_product_batch(proposal,language=language,allowed=set(roles),source_roles=roles)
    proposal_title = "proposal"
    proposal_for_audit = None
    ...
    if revised_payload is not None:
        ...
        validate_financial_prose(revised_proposal.proposal,financial_model)
        validate_export_evidence(revised_proposal.proposal)
        validate_product_batch(revised_proposal.proposal, language=language, allowed=set(), citations=False)
        proposal_title = revised_proposal.proposal.title
        proposal_for_audit = revised_proposal.proposal
        final_markdown = render_formal_b_proposal(revised_proposal.proposal)
    elif state.get("proposal_draft") is not None:
        proposal_draft = ProposalDraft.model_validate(state["proposal_draft"])
        validate_financial_prose(proposal_draft,financial_model)
        validate_export_evidence(proposal_draft)
        validate_product_batch(proposal_draft, language=language, allowed=set(), citations=False)
        proposal_title = proposal_draft.title
        proposal_for_audit = proposal_draft
        final_markdown = render_formal_b_proposal(proposal_draft)
    elif state.get("markdown_preview"):
        final_markdown = clean_business_content(state["markdown_preview"])
        validate_financial_prose({'financial_assumptions':final_markdown},financial_model)
    else:
        raise ValueError(
            "export_node requires revised_proposal, proposal_draft, or markdown_preview."
        )

    final_markdown=resolve_for_acceptance(final_markdown,financial_model)+render_ledger(financial_model)
    policy=current_acceptance()
    if policy is not None:
        for failure in state.get('citation_failures') or []:
            policy.flag(str(failure))
    acceptance = policy.snapshot() if policy is not None else state.get('product_acceptance')

    from workflow.product_output_policy import validate_english_document
    validate_english_document(final_markdown)
    ...
    audit_path=output_path.with_name(output_path.stem+'_audit'+output_path.suffix)
    audit_markdown=render_b_audit_report(proposal_for_audit,state,acceptance=acceptance)
...
def revision_node(
    ...
    from workflow.product_acceptance import current_acceptance, accepted_revision
    if current_acceptance() is not None:
        return accepted_revision(state, llm_client)
    ...
    from workflow.product_output_policy import resolve_output_language, language_instruction
    prompt += language_instruction(resolve_output_language(state.get("user_brief") or {}))
    from workflow.product_finance import FINANCE_INSTRUCTION, render_ledger
    prompt += FINANCE_INSTRUCTION + render_ledger((state.get('finance_assumptions') or {}).get('financial_model'))
